...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (heartbeatData) {
            try {
                const hb = JSON.parse(heartbeatData.value)
                const lastTime = new Date(hb.timestamp).getTime()
                monitorAlive = (Date.now() - lastTime) < 90 * 1000
            } catch {}
        }
        ...
        return NextResponse.json({
            success: true,
            message: monitorAlive
                ? '동기화 요청이 등록되었습니다. 잠시 후 자동으로 시작됩니다.'
                : '동기화 요청은 등록되었지만, 폴더 모니터가 꺼져 있습니다. 로컬 PC에서 ClassIn-Start.bat을 실행해주세요.',
            requestId: data.id,
            monitorAlive
        })
    } catch (error: any) {
