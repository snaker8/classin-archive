...
export default function BatchClassPage() {
    ...
    const onDrop = useCallback((acceptedFiles: File[]) => {
        ...
        acceptedFiles.forEach(file => {
            ...
            // 0. DATE DETECTION (Crucial for hierarchy)
            let date = new Date().toISOString().split('T')[0]
            let dateIndex = -1

            // Search for date folder in path
            for (let i = pathParts.length - 2; i >= 0; i--) {
                const part = pathParts[i]
                // Match formats: 2025-01-01, 25.01.01, etc. (Approximate for UI)
                if (part.match(/^(\d{2,4})[-\.](\d{1,2})[-\.](\d{1,2})/)) {
                    dateIndex = i
                    // Normalize date string (simple version for UI display)
                    date = part.replace(/\./g, '-')
                    if (date.split('-')[0].length === 2) date = '20' + date
                    break
                }
            }

            // 1. PRIORITY: Teacher Board Detection (Folders and Filename)
            let matchedTeacher = null
            for (const part of pathParts) {
                const cleanPart = part.replace(/쌤$/, 'T').replace(/선생님$/, 'T')
                matchedTeacher = teachers.find(t => cleanPart.includes(t.name) || t.name.includes(cleanPart.replace(/T$/, '')))
                if (matchedTeacher) break
            }

            // 2. Class Title Construction (Hierarchy)
            let groupName = 'Uncategorized'
            if (dateIndex > 0) {
                groupName = pathParts[dateIndex - 1]
            } else if (dateIndex === -1 && pathParts.length >= 2) {
                groupName = pathParts[0] // Fallback to root folder if no date found
            }

            let topicParts = dateIndex !== -1 ? pathParts.slice(dateIndex + 1, pathParts.length - 1) : []
            let classTitle = groupName
            if (topicParts.length > 0) {
                classTitle = `${groupName} [${topicParts.join(' ')}]`
            }

            if (matchedTeacher) {
                const key = `TEACHER_${matchedTeacher.id}_${classTitle}_${date}`
                if (!newClassesMap.has(key)) {
                    newClassesMap.set(key, {
                        id: key,
                        studentName: `${matchedTeacher.name} 선생님 (일괄 배포)`,
                        studentId: matchedTeacher.id,
                        date: date,
                        files: [],
                        status: 'pending',
                        message: classTitle,
            ...
            // 3. Normal Student Matching
            let matchedStudent: Profile | undefined
            // Check filename first for student name (standard ClassIn format)
            const possibleNames = filename.split(/[_.\s-]/)
            const firstNamePart = possibleNames[0].replace(/^\d+/, '').trim() // Remove leading number

            if (studentMap.has(firstNamePart)) {
                matchedStudent = studentMap.get(firstNamePart)
            } else {
                // Check folders for student name if not in filename
            ...
            if (matchedStudent) {
                const key = `${matchedStudent.id}_${classTitle}_${date}`
        ...
        async function processTeacherBoard(item: ParsedClass) {
            ...
            try {
                ...
                const { data: classesOnDate, error: classQueryError } = await supabase
                    ...
                    .eq('title', item.message) // message stores the constructed classTitle (e.g. Group [Topic])
                ...
                if (classesOnDate && classesOnDate.length > 0) {
                    ...
                    setClasses(prev => prev.map(c => c.id === item.id ? { ...c, status: 'success', message: `배포 대상 없음 (타겟: ${item.message})` } : c))
                }
